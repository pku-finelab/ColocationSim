 ...
 */
public class SocialNetworkLcEvaluationAtScale {
    public static String DCName = "alicloud";
    public static int USERID = 955;
    public static double beStartAt = 110;
    public static int beRepeatInterval = 3;
    public static int jobNumOnce = 10;
    ...
    static String bePath = "modules\\cloudsim-examples\\src\\main\\java\\org\\cloudbus\\cloudsim\\examples\\colocation\\beJobs.json";
    ...
    public static double memBandwidth = 1333.0;

    public static void main(String[] args){
        Log.setDisabled(true);
        int hostNumStep = 400;
        int hostNumStart = 400;
        int hostNumEnd = 4000;
        int durationStep = 400;
        int durationStart = 100;
        int durationEnd = 1300;
        for (int hostNum = hostNumStart; hostNum <= hostNumEnd; hostNum=hostNum+hostNumStep) {
            for (int duration = durationStart; duration <= durationEnd; duration = duration + durationStep) {
                long start = System.currentTimeMillis();
                double endTick = simOnce(hostNum, duration);
                long end = System.currentTimeMillis();
                double cost = end - start;
                System.out.println("hostNum:"+hostNum+", duration:"+ duration+", cost:"+cost+"ms, lastTick:"+endTick);
            }
        }
    ...
    private static double simOnce(int hostNum, int duration){
        int copysNum = (int) hostNum/33;
        int beRepeat = duration/20;

        int num_user = 1;
        Calendar calendar = Calendar.getInstance();
        CloudSim.init(num_user, calendar, false);
        Datacenter datacenter = createDatacenter(DCName, hostNum);
        ...
        for (int vmId = 0; vmId < hostNum; vmId++) {
        ...
        serviceList = lcBuilder.getServices(copysNum);
        ...
        LoadGenerator lcLoader = lcBuilder.getLoadGenerator(copysNum);
        lcLoader.setDuration(duration);
    ...
    private static ColocationDatacenterBroker createBroker() {
        ...
        try {
            broker = new ColocationDatacenterBroker("ColocationBroker", DCName, Constants.ROUNDROBIN);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    private static Datacenter createDatacenter(String name, int hostNum) {
