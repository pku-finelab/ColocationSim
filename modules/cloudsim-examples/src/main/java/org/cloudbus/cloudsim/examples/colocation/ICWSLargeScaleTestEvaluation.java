 ...
 */
public class ICWSLargeScaleTestEvaluation {
    public static String DCName = "alicloud";
    public static int USERID = 955;
    public static double beStartAt = 110;
    public static int beRepeat = 1;
    public static int beRepeatInterval = 5;
    public static int jobNumOnce = 5;
    public static int vmNum = 100;
    static String lcPath = "modules\\cloudsim-examples\\src\\main\\java\\org\\cloudbus\\cloudsim\\examples\\colocation\\socialnetwork_google.json";
    ...
    static String ALG = Constants.DIAS;
    static String basePath = "E:\\icwsData\\large-cluster\\";
    ...
    public static void main(String[] args) {
        for ( vmNum=250; vmNum <= 250; vmNum = vmNum + 100) {
            int copies = vmNum/50;
            String[] sches = {Constants.DIAS};
            for (String scheduleName : sches) {
                ALG = scheduleName;

                double sla = 0.85;
                double maxSLA = 0.87;
                double step = 0.85;
                for (; sla < maxSLA; sla = sla + step) {
                    String outputPath = basePath + scheduleName + "-" + sla+"-"+vmNum;
                    File targetPath = new File(outputPath);
                    boolean res = targetPath.mkdirs();
                    runOnceWithSLA(sla, outputPath, copies);
                }
            }
        }
    }
    public static void runOnceWithSLA(double sla, String outPath, int copies){
        long start = System.currentTimeMillis();
        int num_user = 1;
        Calendar calendar = Calendar.getInstance();
        CloudSim.init(num_user, calendar, false);
        Datacenter datacenter = createDatacenter(DCName);
        ColocationDatacenterBroker broker = createBroker(sla, outPath);
        int totalBENumbers= beRepeat*jobNumOnce;
        broker.setBeSchedulerExitNumber(totalBENumbers);

        broker.setOutput(false);
        QosCenter qosCenter = new QosCenter(sla);
        ...
        serviceList = lcBuilder.getServices(copies);
        ...
        LoadGenerator lcLoader = lcBuilder.getLoadGenerator(copies);
    ...
    private static ColocationDatacenterBroker createBroker(double sla, String outputPath) {
